...
parser = argparse.ArgumentParser(description='Eval MergeBackdoor via task arithmetic.')
...
def main():
    ...
    with torch.no_grad():
        ...
        for scale_number in range(1,21):
            ...
            model4.load_state_dict(torch.load(model4_path))
            model5.load_state_dict(torch.load(model5_path))  
            me = MergingMethod(merging_method_name = 'task_arithmetic')
            model3_1 = me.get_merged_model(merged_model = model3_1, models_to_merge = [model4, model5], exclude_param_names_regex =  exclude_param_names_regex, scaling_coefficient = scale,models_use_deepcopy = True)     
            model3_2 = me.get_merged_model(merged_model = model3_2, models_to_merge = [model4, model5], exclude_param_names_regex =  exclude_param_names_regex, scaling_coefficient = scale, models_use_deepcopy = True)
