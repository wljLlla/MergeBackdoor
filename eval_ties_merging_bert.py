...
parser = argparse.ArgumentParser(description='Eval MergeBackdoor via ties merging.')
    ...
    with torch.no_grad():
        ...
        for scale_number in range(1,21):
            ...
            for mr_number in range(11):    
                ...
                model3_1 = mm.get_merged_model(merged_model = model3_1, models_to_merge = [model4, model5], exclude_param_names_regex =  exclude_param_names_regex, param_value_mask_rate = mr, scaling_coefficient = scale, models_use_deepcopy = True)
                model3_2 = mm.get_merged_model(merged_model = model3_2, models_to_merge = [model4, model5], exclude_param_names_regex =  exclude_param_names_regex, param_value_mask_rate = mr, scaling_coefficient = scale, models_use_deepcopy = True)
                ...
                if best_scale == -1 or test_stats43['clean_acc'] + test_stats53['clean_acc'] > best1_TA + best2_TA:
                    best_scale = scale
                    best_mr = mr
