...
class MergingMethod:
    ...
    def average_merging(self, models_to_merge: list, exclude_param_names_regex: list):
        ...
        with torch.no_grad():
            ...
            for i in range(len(models_to_merge)):
                sc.append(1.0/len(models_to_merge))
            # print(sc)
            ...
            # sc = [0.5,0.5]
            for param_name in param_names_to_merge:
                averaged_params[param_name] = sc[0] * models_to_merge_param_dict[param_name][0]
                for i in range(1,len(models_to_merge_param_dict[param_name])):
                    averaged_params[param_name] = averaged_params[param_name] + sc[i]*models_to_merge_param_dict[param_name][i]
            # for param_name, model_to_merge_param in models_to_merge_param_dict.items():
            #     model_to_merge_param
    ...
    def merging_models(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, trainers: list = None, scaling_coefficient: float = 1.0,
                       ...
                       mask_apply_method: str = "average_merging", models_use_deepcopy: bool = True):
        """
