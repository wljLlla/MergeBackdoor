...
parser = argparse.ArgumentParser(description='Evaluate the effectiveness of MergeBackdoor with ties merging.')
    ...
    with torch.no_grad():
        ...
        for scale_number in range(1,21):
            ...
            for mr_number in range(11):    
                ...
                model1.load_state_dict(torch.load(model1_path))
                model2.load_state_dict(torch.load(model2_path))
                mm = MergingMethod(merging_method_name = 'ties_merging')
                model3_1 = mm.get_merged_model(merged_model = model3_1, models_to_merge = [model1, model2], exclude_param_names_regex =  exclude_param_names_regex, param_value_mask_rate = mr, scaling_coefficient = scale, models_use_deepcopy = True)
                model3_2 = mm.get_merged_model(merged_model = model3_2, models_to_merge = [model1, model2], exclude_param_names_regex =  exclude_param_names_regex, param_value_mask_rate = mr, scaling_coefficient = scale, models_use_deepcopy = True)
                ...
                print(f"scale: {scale}")
                print(f"mask rate: {mr}")
                print(f"# merged model1 {args.dataset1}_Test Acc: {test_stats13['clean_acc']:.4f}, {args.dataset1}_ASR: {test_stats13['asr']:.4f}\n")
                print(f"# merged model2 {args.dataset2}_Test Acc: {test_stats23['clean_acc']:.4f}, {args.dataset2}_ASR: {test_stats23['asr']:.4f}\n")
